...
from autosted.detection.spot_util import detect_blobs, detect_blobs_find_pairs
from autosted.utils.coordinate_utils import pixel_to_physical_coordinates
...
class SimpleSingleChannelSpotDetector:
    ...
    def to_world_coordinates(self, localizations, settings, ignore_dim, img_shape):
        ...
        offset_image = np.array(
            [
                get_path_from_dict(settings, off_path, keep_structure=False)
                for off_path in self.offset_parameter_paths
            ],
            dtype=float,
        )
        psz_image = np.array(
            [
                get_path_from_dict(settings, off_path, keep_structure=False)
                for off_path in self.pixel_size_parameter_paths
            ],
            dtype=float,
        )
    ...
    def __call__(self):
        ...
        if (data.num_configurations <= self.configuration) or (
            data.num_channels(self.configuration) < max(self.channels)
        ):
            raise ValueError(
                "required images not present. TODO: fail gracefully/skip here"
            )
        ...
        for channel, threshold in zip(self.channels, self.thresholds):
            ...
            # do detection
            locs_for_channel = detect_blobs(
                img_squeezed,
                sigmas,
                threshold,
                normalize=False,
                threshold_rel_median=self.median_threshold,
                med_radius=self.median_radius,
                refine=self.refine_detections,
            )
        ...
        corrected = (
            self.to_world_coordinates(locs, settings, ignore_dim, img.shape)
            if len(locs) > 1
            else []
        )
        ...
        self.logger.info(
            ...
            + ": found {} spots. pixel coordinates:".format(len(locs))
        )
        for loc in locs:
        ...
        self.logger.info(
            self.__class__.__name__
            + ": found {} spots. physical coordinates:".format(len(locs))
        )
        ...
        if not self.return_parameter_dict:
            ...
            return ValuesToSettingsDictCallback(
                lambda: corrected, self.offset_parameter_paths
            )()
...
class LegacySpotPairFinder:
    ...
    def to_world_coordinates(self, detections_pixel, setts, ignore_dim):
        ...
        offsets = np.array(
            [
                get_path_from_dict(setts, path, False)
                for path in self.offset_parameter_paths
            ],
            dtype=float,
        )
        fov_lengths = np.array(
            [
                get_path_from_dict(setts, path, False)
                for path in self.fov_length_parameter_paths
            ],
            dtype=float,
        )
        pixel_sizes = np.array(
            [
                get_path_from_dict(setts, path, False)
                for path in self.pixel_size_parameter_paths
            ],
            dtype=float,
        )
    ...
    def __call__(self):
        ...
        pairsRaw = detect_blobs_find_pairs(
            stack1,
            stack2,
            self.sigma,
            self.thresholds,
        ...
        self.logger.info(
            ": found {} spot pairs. pixel coordinates:".format(len(pairsRaw))
        )
        for pr in pairsRaw:
            self.logger.info(pr)
        ...
        self.logger.info(
            self.__class__.__name__
            + ": found {} spot pairs. offsets:".format(len(pairsRaw))
        )
        for pc in corrected:
            self.logger.info(pc)
...
class PairedLegacySpotPairFinder(LegacySpotPairFinder):
    ...
    def __call__(self):
        ...
        if self.plot_detections:
            self.do_plot(
                [list((np.array(p[0]) + np.array(p[1])) / 2) for p in pairsRaw],
                stack1,
                stack2,
            )

        ignore_dim = np.array([d for d in stack1.shape]) == 1
        corrected_1 = self.to_world_coordinates(
            [p[0] for p in pairsRaw], setts, ignore_dim
        )
        corrected_2 = self.to_world_coordinates(
            [p[1] for p in pairsRaw], setts, ignore_dim
        )
