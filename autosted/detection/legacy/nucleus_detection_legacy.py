...
def nnet_seg_outer(
    ...
    res = []
    img_reordered = np.transpose(
        img, [axis] + [ax for ax in range(len(img.shape)) if ax != axis]
    )
    for rprop in regionprops(labels):
        ...
        max_z_refined = refine_point(
            ndi.gaussian_filter(sums, 2, mode="constant"), [max_z]
        )
...
def stardist_midplane_detection(
    img,
    model,
    scale_factor=0.5,
    prob_tresh=0.8,
    ...
    return nnet_seg_outer(
        img, stardist_seg_fun, scale_factor, axis, flt, do_plot, ignore_border, bg_val
    )


def cellpose_midplane_detection(
    img,
    model,
    scale_factor=0.5,
    flow_tresh=0.2,
    ...
    def cellpose_seg_fun(img):
        segs, *_ = model.eval(
            [img], flow_threshold=flow_tresh, diameter=diameter, channels=[0, 0]
        )
        return segs[0]

    return nnet_seg_outer(
        img, cellpose_seg_fun, scale_factor, axis, flt, do_plot, ignore_border, bg_val
    )


def nucleus_midplane_detection(
    img, axis=0, flt=None, do_plot=False, ignore_border=True, bg_val=None, n_classes=2
):
    """
    ...
    if bg_val is None:
        ...
        print(test.shape)
        feat = np.dstack(
            [mip[mip2 != bg_val], blur[mip2 != bg_val], edge[mip2 != bg_val]]
        )
        print(feat.shape)
...
class SimpleNucleusMidplaneDetector:
    ...
    def __call__(self):
        ...
        if data is None:
            if self.verbose:
                print(
                    self.__class__.__name__ + ": ERROR: no image for nucleus detection"
                )
            return []
        ...
        if self.use_stage:
            offsets_old = np.array(
                [
                    get_path_from_dict(settings, path, False)
            ...
            offsets_old = np.array(
                [
                    get_path_from_dict(settings, path, False)
                    for path in OFFSET_SCAN_PARAMETERS
                ],
                dtype=float,
            )
        fov_lengths_old = np.array(
            [
                get_path_from_dict(settings, path, False)
                for path in FOV_LENGTH_PARAMETERS
            ],
            dtype=float,
        )
        pixel_sizes_old = np.array(
            [
                get_path_from_dict(settings, path, False)
                for path in PIXEL_SIZE_PARAMETERS
            ],
            dtype=float,
        )
        ...
        for ymin, xmin, ymax, xmax, zmid in midplanes:
            ...
            off = pixel_to_physical_coordinates(
                off,
                offsets_old,
                pixel_sizes_old,
                fov_lengths_old,
                ignore_dimension=ignore_dimensions,
            )
            fov = [
                None,
                (ymax - ymin) * pixel_sizes_old[1] * self.expand,
                (xmax - xmin) * pixel_sizes_old[2] * self.expand,
            ]
...
class StarDistNucleusMidplaneDetector(SimpleNucleusMidplaneDetector):
    def __init__(
        self, data_source_callback, scale_factor=0.5, prob_thresh=0.8, **kwargs
    ):
    ...
    def midplane_detection_fun(self, img):
        return stardist_midplane_detection(
            img,
            self.model,
            self.scale_factor,
            self.prob_tresh,
...
class CellposeNucleusMidplaneDetector(SimpleNucleusMidplaneDetector):
    ...
    def midplane_detection_fun(self, img):
        return cellpose_midplane_detection(
            img,
            self.model,
            self.scale_factor,
            self.flow_tresh,
