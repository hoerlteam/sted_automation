...
class ParameterSanitizer:
    ...
    pattern_parent = re.compile(
        "(?:Internal error: )?Invalid argument for (?:device|parameter) '(.*?)'"
    )
    pattern_last = re.compile("No parameter '(.*?)'")
    # new error message that appeared with the addition of 'Powerswitch'
    unknown_device_error = re.compile(
        "Internal error: Unknown device or parameter '(.*?)'"
    )
    ...
    def parse_runtime_error(self, e):
        ...
        if m_unknown_device:
            ...
            print(
                "WARNING: parameter {} cannot be set, ignoring from here on".format(
                    par_dev_unknown
                )
            )
            return
    ...
    def del_recursive(d, params):
        ...
        if len(params) == 0:
            warn(
                "WARNING: tried to remove empty parameter (this is probably okay, but should not happen - look into it!)"
            )
            return
...
class ImspectorConnection:
    ...
    def get_current_data(self):
        ...
        for name in self.imspector.active_measurement().stack_names():
            stack_data.append(
                np.copy(self.imspector.active_measurement().stack(name).data())
            )
    ...
    def get_n_channels(parameters):
        return len(
            get_path_from_dict(parameters, "ExpControl/measurement/channels", False)
        )
    ...
    def run_current_measurement(self):
        ...
        offsets_scan_global = [
            get_path_from_dict(params, path, False)
            for path in OFFSET_SCAN_GLOBAL_PARAMETERS
        ]
        fov_length = [
    ...
    def blocking_imspector_run(imspector, measurement):
        ...
        warn(
            "blocking_imspector_run is deprecated and no longer necessary",
            DeprecationWarning,
        )
