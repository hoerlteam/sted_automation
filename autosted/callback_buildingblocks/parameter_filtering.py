...
from autosted.utils.parameter_constants import LOCATION_PARAMETERS
...
class ParameterFilter:
    ...
    def keep_parameters(self):
        ...
        for measurement_parameters_old in self.wrapped_callback():
            ...
            for (
                acquisition_updates_old,
                hardware_updates_old,
            ) in measurement_parameters_old:
                ...
                acquisition_updates_new = (
                    ...
                    else acquisition_updates_old
                )
                hardware_updates_new = (
                    {} if self.hardware_parameters is not None else hardware_updates_old
                )
                ...
                if self.acquisition_parameters is not None:
                    for acquisition_update_key in self.acquisition_parameters:
                        old_value = get_path_from_dict(
                            acquisition_updates_old, acquisition_update_key, False
                        )
                        acquisition_updates_new = merge_dicts(
                            acquisition_updates_new,
                            (
                                generate_nested_dict(old_value, acquisition_update_key)
                ...
                if self.hardware_parameters is not None:
                    for hardware_update_key in self.hardware_parameters:
                        old_value = get_path_from_dict(
                            hardware_updates_old, hardware_update_key, False
                        )
                        hardware_updates_new = merge_dicts(
                            hardware_updates_new,
                            (
                                generate_nested_dict(old_value, hardware_update_key)
                                if old_value is not None
                                else {}
                            ),
                        )
                measurement_parameters_new.append(
                    (acquisition_updates_new, hardware_updates_new)
                )
    ...
    def remove_parameters(self):
        ...
        for measurement_parameters_old in self.wrapped_callback():
            ...
            for (
                ...
                if self.acquisition_parameters is not None:
                    for acquisition_update_key in self.acquisition_parameters:
                        acquisition_updates_new = remove_path_from_dict(
                            acquisition_updates_new, acquisition_update_key
                        )
                ...
                if self.hardware_parameters is not None:
                    for hardware_update_key in self.hardware_parameters:
                        hardware_updates_new = remove_path_from_dict(
                            hardware_updates_new, hardware_update_key
                        )
...
class LocationKeeper(ParameterFilter):
    ...
    def __init__(self, wrapped_callback):
        ...
        super().__init__(
            ...
            acquisition_parameters=LOCATION_PARAMETERS,
        )
