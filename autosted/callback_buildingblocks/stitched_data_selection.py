...
from autosted.utils.dict_utils import merge_dicts, generate_nested_dict, get_parameter_value_array_from_dict
...
class StitchedNewestDataSelector(NewestDataSelector):
    ...
    def __call__(self):
        ...
        min_r, len_r = min_r[np.logical_not(singleton_dims)], len_r[np.logical_not(singleton_dims)]
        ...
        for idx in indices_same_level:
            ...
            min_i, len_i = min_i[np.logical_not(singleton_dims)], len_i[np.logical_not(singleton_dims)]
        ...
        new_len = np.array(refill_ignored_dimensions(len_stitched_half, singleton_dims)) * 2 * pixel_sizes
        ...
        for i, (p_off, p_len) in enumerate(zip(self.offset_parameter_paths, FOV_LENGTH_PARAMETERS)):
            stitch_setts = merge_dicts(stitch_setts, generate_nested_dict(new_len[i], p_len))
            stitch_setts = merge_dicts(stitch_setts, generate_nested_dict(offs_to_use[i] + additional_off[i], p_off))

        # stitched image to Imspector shape
        # add singleton dimensions for time as well as zyx dimensions that were squeezed

        res_img = np.expand_dims(stitched, tuple([i for (i, singleton) in enumerate([True] + singleton_dims) if singleton]))
